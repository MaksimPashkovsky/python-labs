...
FROM base AS integration-tests-http

WORKDIR /usr/src/math-server
COPY test-requirements.txt /usr/src/math-server
RUN pip install --no-cache-dir -r test-requirements.txt

COPY src/mathserver/tests/integration_tests_http.py /usr/src/math-server/tests/

CMD ["pytest", "tests/integration_tests_http.py"]

FROM base AS integration-tests-socket

WORKDIR /usr/src/math-server
COPY test-requirements.txt /usr/src/math-server
RUN pip install --no-cache-dir -r test-requirements.txt

COPY src/mathserver/tests/integration_tests_socket.py /usr/src/math-server/tests/
